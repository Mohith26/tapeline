...
#   client    (sends orders, drops its connection halfway and resumes the session)
...
./build/exchange --gw-port "$GW" --feed-addr "$FEED_ADDR" --feed-port "$FEED_PORT" --retrans-port "$RT" \
  ...
  --state-out results/exchange_state.json --idle-exit-ms 1500 --quiet &
EX=$!
sleep 0.3
...
FD=$!
sleep 0.2
./build/client --port "$GW" --orders "$ORDERS" --symbols "$SYMBOLS" --window "$WINDOW" \
  --resume-at $((ORDERS / 2)) --out results/client.json > /dev/null
wait "$EX"
wait "$FD"
